...
Feature Implementation is a workflow for building new features, enhancements, and general development work using an AI coding agent backed by CoreStory's code intelligence. The agent uses CoreStory's MCP tools to understand your system's architecture, identify implementation patterns, and navigate to the right code locations before writing a single line â€” then implements the feature using a test-driven approach.

CoreStory serves two roles in this workflow:

- **Oracle** â€” answers questions about system architecture, design patterns, naming conventions, and invariants. The agent queries CoreStory to understand _how the system works_ and _what patterns to follow_ before coding.
- **Navigator** â€” points to specific files, extension points, base classes, and data structures. The agent queries CoreStory to understand _where to implement_ and _what to reuse_.
...
The workflow has six phases. Each phase builds on the previous one. Skipping phases â€” especially the oracle and navigator phases â€” increases the risk of architectural misalignment, missed patterns, and avoidable rework.
...
| `create_conversation` | Phase 1 â€” create a persistent investigation thread |
| `send_message` | Phases 2â€“5 â€” all queries to CoreStory |
...
**1.1 â€” Get ticket details.** If a ticketing MCP is connected, the agent fetches the ticket directly:
...
**1.2 â€” Select CoreStory project.**
...
**1.3 â€” Create an implementation conversation.**
...
This conversation persists across the session and captures the full chain of queries and responses â€” valuable institutional knowledge for future work.
...
**2.1 â€” Query system architecture.** Ask CoreStory about the feature area:
...
**2.2 â€” Query design patterns and conventions.**
...
**2.3 â€” Query historical context.**
...
**3.1 â€” Identify extension points.**
...
**3.2 â€” Understand data structures.**
...
**3.3 â€” Identify reference implementations.**
...
This is one of the highest-value queries. A good reference implementation gives the agent a concrete template to follow â€” not just abstract patterns, but actual working code in the same codebase.
...
Tests come before implementation code. This is non-negotiable in the workflow â€” it ensures acceptance criteria are codified, requirements are clearly understood, and regressions are caught immediately.

**4.1 â€” Write acceptance tests** from the criteria gathered in Phase 1, following the architecture patterns from Phase 2 and data structures from Phase 3:
...
**4.2 â€” Validate tests with CoreStory.**
...
**4.3 â€” Verify tests fail.** Run the tests and confirm they fail. If they pass, the feature already exists or the tests are wrong â€” clarify with CoreStory.

**4.4 â€” Implement the feature** following patterns from Phase 2:
...
**4.5 â€” Verify tests pass.** Run the specific test file and confirm all tests go green.

**4.6 â€” Validate implementation with CoreStory.**
...
**5.1 â€” Identify edge cases** by querying CoreStory:
...
**5.2 â€” Run the full test suite.** Ensure all new tests pass and no existing tests broke. If regressions appear, the implementation introduced unintended side effects â€” revise the approach.

**5.3 â€” Performance and security validation** (when applicable). Add performance tests for features with latency/throughput requirements. Add security tests for features handling sensitive data or authentication.

---

### Phase 6: Completion & Knowledge Capture

**Objective:** Close the loop â€” commit, document, and preserve knowledge.

**6.1 â€” Update ticket** (if ticketing MCP is connected). Post an implementation summary with files created/modified, pattern followed, test count, and coverage.

**6.2 â€” Commit with rich context.** The commit message should document the feature, implementation approach, architectural alignment, and testing scope:
...
**6.3 â€” Preserve the CoreStory conversation.**
...
**2. (Optional) Connect a ticketing system MCP.** For automatic ticket intake, add an MCP server for your issue tracker. Examples:
...
Set the `GITHUB_PERSONAL_ACCESS_TOKEN` environment variable, or pass it inline:

```bash
claude mcp add github --env GITHUB_PERSONAL_ACCESS_TOKEN=ghp_xxxx \
  -- npx -y @modelcontextprotocol/server-github
```

Jira, Linear, and Azure DevOps MCP servers follow the same pattern. Check the [MCP server registry](https://github.com/modelcontextprotocol/servers) for current packages and configuration.
...
description: >
  ...
  "Resolve ENG-456"). Do NOT use for bug fixes â€” use the fix-bug skill instead.
---
...
**Query 2 â€” Design patterns and conventions:**
...
3. Verify all tests FAIL. If they pass, the feature may already exist â€”
...
3. Run FULL test suite â€” all tests must pass, no regressions.
...
```
Implement ticket #6992 â€” add webhook support for project events
Build feature JIRA-456 â€” user notification preferences
```
...
2. Identify acceptance criteria â€” ask for them if not provided
...
### Phase 2: Oracle Phase â€” Understand Architecture
...
### Phase 3: Navigator Phase â€” Plan Implementation
...
3. Run full test suite â€” no regressions
...
- **Test-first always**: failing tests â†’ implement â†’ verify passes
...
3. Plan implementation â€” files to create/modify, data structures, reference patterns
...
- Identify acceptance criteria â€” ask if missing
...
3. Rename CoreStory conversation â†’ "COMPLETED"
4. Report results
...
```
@implement-feature Implement ticket #6992 â€” add webhook support
```
...
- For complex features, the droid's CoreStory queries may produce long streaming responses â€” this is expected.
...
- `CoreStory:create_conversation` â€” start implementation thread
...
2. Identify acceptance criteria â€” ask if missing
...
## Phase 4: TDD Implementation
Write failing tests â†’ verify fail â†’ validate with CoreStory â†’ implement â†’ verify pass â†’ validate implementation.

## Phase 5: Feature Completion
Edge case tests â†’ full suite â†’ performance/security tests if needed.

## Phase 6: Completion
Update ticket â†’ structured commit ("Feat: ...") â†’ rename conversation "COMPLETED" â†’ report.
...
**Start with the oracle, not the editor.** The most common mistake is jumping straight to implementation. Even experienced developers benefit from the oracle phase â€” CoreStory often surfaces patterns, utilities, and conventions that aren't obvious from reading code.
