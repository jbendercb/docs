...
This is a companion guide to the [Spec-Driven Development](/playbooks/spec-driven-development) playbook. It covers how to integrate GitHub's **Spec Kit** â€” a specification-driven development framework â€” with CoreStory's architecture-grounded SDD workflow.
...
|---------------------|-----------------|---------------------|
| Phase 1 â€” Ground | `/speckit.constitution` | CoreStory findings feed the constitution |
| Phase 2 â€” Specify | `/speckit.specify` | Delta spec constrains the Spec Kit specification |
| Phase 3 â€” Validate | *(no Spec Kit equivalent)* | CoreStory-only â€” validate the `.specify` artifacts |
| Phase 4 â€” Plan | `/speckit.plan` + `/speckit.tasks` | Validated spec feeds planning and task breakdown |
| Phase 5 â€” Implement | `/speckit.implement` | Standard TDD implementation |
| Phase 6 â€” Verify & Capture | *(no Spec Kit equivalent)* | CoreStory-only â€” verify invariants and capture knowledge |

The critical insight: **Phases 3 and 6 have no Spec Kit equivalent.** These are CoreStory-only phases â€” architectural validation and invariant verification. This is the gap that CoreStory fills. Spec Kit structures the artifacts; CoreStory validates them against reality.
...
**Output:** `.specify/memory/constitution.md` â€” a constitution with specific file references, real patterns, and actual constraints.
...
### Phase 2: Specify â†’ Specification
...
Delta â€” new/modified components:
...
**Output:** `.specify/memory/features/{name}/spec.md` â€” a specification that inherits the delta structure from the main playbook.

### Phase 3: Validate (CoreStory Only)

Spec Kit has no validation command. This is the gap.

After Spec Kit generates the specification artifact, submit it to CoreStory for architectural pre-mortem â€” exactly as described in the main playbook's Phase 3. Paste the contents of `.specify/memory/features/{name}/spec.md` into the CoreStory query:
...
Run `/speckit.implement` and let the agent follow the task list. The main playbook's Phase 5 applies in full â€” write failing tests, implement in dependency order, validate with CoreStory at each step.
...
### Claude Code â€” Skill File Addition
...
- Phase 1: After CoreStory grounding â†’ run `/speckit.constitution`
- Phase 2: After delta spec â†’ run `/speckit.specify` with invariants/reuse/delta
- Phase 3: Validate `.specify/memory/features/{name}/spec.md` with CoreStory
- Phase 4: After CoreStory mapping â†’ run `/speckit.plan` then `/speckit.tasks`
...
### GitHub Copilot â€” Instructions Addition
...
Phase mapping: Ground â†’ `/speckit.constitution`, Specify â†’ `/speckit.specify`,
Plan â†’ `/speckit.plan` + `/speckit.tasks`, Implement â†’ `/speckit.implement`.
Validate and Verify phases are CoreStory-only (no Spec Kit equivalent).
```

### Cursor â€” Rule Addition
...
- Phase 1: CoreStory grounding â†’ `/speckit.constitution`
- Phase 2: Delta spec â†’ `/speckit.specify`
- Phase 3: CoreStory validation of `.specify` artifacts (no Spec Kit command)
- Phase 4: CoreStory mapping â†’ `/speckit.plan` â†’ `/speckit.tasks`
...
- Phase 1 â†’ `/speckit.constitution`
- Phase 2 â†’ `/speckit.specify`
- Phase 4 â†’ `/speckit.plan` + `/speckit.tasks`
- Phase 5 â†’ `/speckit.implement`
...
**CoreStory before Spec Kit at every phase.** The pattern is always: query CoreStory â†’ get grounded context â†’ feed context into Spec Kit command. Never run a Spec Kit command without CoreStory context first.
...
**All other issues** â€” CoreStory generic responses, project not found, agent not following the workflow â€” are covered in the main [Spec-Driven Development](/playbooks/spec-driven-development) playbook's Troubleshooting section.
