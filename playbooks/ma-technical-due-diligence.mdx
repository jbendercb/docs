...
Technical due diligence on acquisition targets traditionally relies on CTO interviews, manual code sampling, and documentation review â€” covering less than 10% of the actual codebase. Critical risks hide in the 90% nobody reads: hardcoded secrets in utility modules, GPL dependencies buried three layers deep, PII flowing unencrypted through forgotten microservices.

CoreStory changes this equation. By ingesting and semantically understanding the target's entire codebase, it gives your diligence team an AI-powered analyst that can answer specific risk questions against the full codebase â€” not a sample. The agent operates as an **Oracle** (explaining system behavior, architectural patterns, and data flows) and a **Navigator** (pointing to specific files, methods, and code paths where risks live).
...
**What you'll get:** A structured workflow for interrogating a target codebase across four risk domains â€” technical debt, security, data/PII compliance, and integration complexity â€” producing auditable findings with specific file-level evidence.
...
| `send_message` | Interrogate the codebase â€” the primary investigation tool |
...
1. **Setup** â€” Confirm the target project, review synthesized specs for architectural orientation, and create named conversation threads for each workstream.
2. **Interrogate** â€” Use `send_message` to ask specific risk questions. CoreStory answers from semantic understanding of the full codebase, citing specific files and code paths.
3. **Cross-reference** â€” Validate critical findings against the actual source code. CoreStory provides the file paths and context; the agent (or your team) confirms.
4. **Synthesize** â€” Compile findings into structured reports. Conversation history provides the audit trail.

### Oracle Before Navigator

The "Oracle before Navigator" principle is especially important in diligence. Before searching for specific vulnerabilities or code paths, first use CoreStory to understand how the system is *designed* to work â€” its intended architecture, data flow patterns, and security model. This baseline makes it far easier to spot deviations, shortcuts, and risks.
...
The agent calls `list_projects` and returns your available projects. Confirm the correct project before proceeding â€” this is a critical safety step when multiple targets may be under evaluation simultaneously.
...
Store the conversation_id â€” we'll use this thread for all risk-related queries.
```
...
separation of concerns â€” god classes, circular dependencies, modules with
...
or unmaintained â€” no recent commits, no test coverage, referenced but not
...
â€” GPL, AGPL, SSPL, or other copyleft licenses that could create obligations
...
CoreStory provides findings with specific file paths and code context. For critical findings â€” especially security issues, licensing risks, and PII exposure â€” validate against the actual source code.
...
1. Executive Summary â€” top 3-5 findings that materially affect deal risk or valuation
2. Critical Risks â€” security vulnerabilities, licensing exposure, PII compliance gaps
   (each with file-level evidence and severity rating)
3. Technical Debt â€” end-of-life dependencies, architectural issues, maintainability concerns
4. Architecture Overview â€” component map, technology stack, external dependencies
5. Integration Assessment â€” complexity of merging with acquirer's platform, friction points
6. Recommendations â€” remediation priorities and estimated effort for post-close planning
```
...
**Timeline:** 24â€“48 hours with CoreStory (vs. 2â€“4 weeks traditional).
...
**Scenario:** The deal is closing. The acquiring engineering team needs an integration blueprint â€” what overlaps, what conflicts, and where the friction will be.
...
Use send_message to ask:
- "Describe the complete system architecture â€” services, databases,
...
Use send_message to ask:
- "Map all data flows that handle PII â€” customer registration,
...
Use send_message to ask:
- "Identify all hardcoded secrets â€” API keys, passwords, tokens,
...
- Generate a "PII & Security Compliance Report" covering:
  1. PII inventory â€” what data, where stored, encryption status
  2. Data flow map â€” collection through deletion lifecycle
  3. Compliance gaps â€” unencrypted PII, missing deletion flows,
     third-party data sharing without controls
  4. Security findings â€” hardcoded secrets, missing validation,
...
Effective diligence queries are specific and evidence-oriented. They ask for file paths, concrete examples, and traceable findings â€” not summaries.
...
```
Query 1: "Map the system architecture â€” services, databases, and external integrations."
...
**Start with the synthesized specs.** Use `get_project_techspec` and `get_project_prd` before diving into `send_message` queries. The specs give you architectural vocabulary â€” service names, data model names, API endpoint patterns â€” that make your queries far more specific and productive.
...
**Cross-reference critical findings.** CoreStory's analysis is based on its ingestion snapshot. For findings that materially affect deal terms â€” licensing poison pills, PII exposure, critical security vulnerabilities â€” always validate against the current source.
...
Your queries are too broad. Include specific anchors â€” service names, module names, technology names, or code patterns. After reviewing the Technical Specification, use the vocabulary it provides (component names, data model names) in your queries.
