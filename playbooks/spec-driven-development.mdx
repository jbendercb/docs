...
Spec-driven development (SDD) is gaining traction as teams realize that handing an AI agent a vague ticket and hoping for the best doesn't scale. Tools like GitHub's Spec Kit, Kiro, and Tessl have emerged to impose structure â€” write a spec first, then generate code from it. The structure is valuable. But every one of these tools shares a fundamental blind spot: they don't actually know how your system works. They scan files, infer patterns, and guess. The specs they produce are architecturally ungrounded â€” and ungrounded specs produce code that reinvents existing services, violates established patterns, and breaks invariants the spec author didn't know existed.

This playbook is CoreStory's answer to the grounding problem. It's a six-phase workflow where your AI agent queries CoreStory's code intelligence to understand the real architecture *before* writing a single line of specification â€” then validates the spec *against* that architecture before implementation begins. The result is a **delta specification** â€” only what changes, constrained by what exists â€” rather than an aspirational document that may contradict reality.

CoreStory serves two roles in this workflow:

- **Oracle** â€” answers questions about architectural patterns, naming conventions, business rules, invariants, and design decisions. This is context synthesized from PRDs, tech specs, user stories, and code history. It's the institutional knowledge that normally lives only in the heads of long-tenured engineers.
- **Navigator** â€” points to specific files, services, base classes, extension points, and data structures. Instead of guessing where new code should go or what already exists, the agent gets directed guidance grounded in the actual codebase.
...
**How this relates to other playbooks:** The Implementation phase (Phase 5) deliberately stays lightweight because the Bug Resolution and Feature Implementation playbooks cover test-driven implementation in depth. This playbook's unique contribution is Phases 1â€“4 (Ground, Specify, Validate, Plan) â€” the specification workflow that precedes implementation. Cross-reference the other playbooks when you reach Phase 5.
...
- **AI coding agent** â€” this playbook includes implementation guides for Claude Code, GitHub Copilot, Cursor, and Factory.ai. The generic workflow applies to any MCP-capable agent.
- **Optional:** Ticketing system MCP (GitHub Issues, Jira, Linear, Azure DevOps) for automatic ticket intake
- **Optional:** A companion SDD framework (Spec Kit, Kiro, etc.) â€” this playbook works standalone, but companion integrations are covered in separate guides
...
The workflow has six phases. The first four produce and validate the specification; the last two implement and verify it. Phases 1 and 3 â€” Ground and Validate â€” are what distinguish this from generic SDD. They can't exist without architectural knowledge, which is why no standalone SDD tool offers them.
...
| 3 | Validate | Architectural pre-mortem â€” submit spec for conflict detection | Oracle |
...
| Tool | When Used |
|------|-----------|
| `list_projects` | Phase 1 â€” select the right project |
| `create_conversation` | Phase 1 â€” create a persistent specification thread |
| `send_message` | Phases 1â€“6 â€” all queries to CoreStory |
| `get_project_prd` | Phase 1 â€” access product requirements for grounding |
| `get_project_techspec` | Phase 1 â€” access technical specifications |
| `rename_conversation` | Phase 6 â€” mark conversation as completed |
...
Traditional SDD produces **greenfield specifications** â€” documents that describe the entire feature from scratch as if no codebase existed. This creates three problems:
...
Delta specification solves all three. By grounding in the real architecture first, the spec only describes *what changes* â€” new components, modified behaviors, extended interfaces. Everything that stays the same is referenced, not re-specified.
...
This phase is the foundation. Without it, every subsequent phase operates on assumptions â€” and assumptions are where specs go wrong. The output is a grounded understanding captured in a CoreStory conversation thread, not a static constitution document that drifts out of sync with the code.

**1.1 â€” Gather requirements.** If a ticketing MCP is connected, fetch the ticket:
...
**1.2 â€” Select CoreStory project and create conversation.**
...
'Spec: #412 â€” Add webhook notification system'"
```

This conversation persists across the entire specification lifecycle. It captures not just what was decided, but the architectural reasoning behind every decision â€” institutional knowledge that outlasts any static spec document.

**1.3 â€” Query architectural patterns.**
...
**1.4 â€” Query existing services and reusable components.**
...
**1.5 â€” Query invariants and constraints.**
...
**Phase 1 produces:** A CoreStory conversation containing architectural patterns, existing services to reuse, invariants to preserve, and design history â€” the grounded context that constrains everything that follows.

---

### Phase 2: Specify

**Objective:** Write a delta specification â€” requirements constrained by what exists.
...
**2.1 â€” Define invariants (what must NOT change).**
...
- All outbound webhook payloads must exclude PII fields defined in
  DataPrivacyConfig
- Webhook delivery must be idempotent â€” duplicate delivery must not
...
**2.2 â€” Define reuse (what already exists).**
...
- `EventBus` â€” subscribe to domain events (already handles user.created,
  order.completed, etc.)
- `HttpClient` â€” outbound HTTP with retry, circuit breaking, rate limiting
- `SerializerFactory` â€” JSON/XML payload serialization
- `NotificationPreferences` â€” user-level notification opt-in/opt-out
  (extend for webhook channel)
- `AuditLogger` â€” log all outbound webhook attempts for compliance
```

**2.3 â€” Define the delta (what changes).**

Now â€” and only now â€” specify the new components, modified interfaces, and new behaviors:
...
- POST /api/v2/webhooks â€” create subscription (follows existing REST conventions)
- DELETE /api/v2/webhooks/{id} â€” remove subscription
- GET /api/v2/webhooks â€” list subscriptions (paginated, same pattern as /api/v2/notifications)

## Modified Components

### EventBus
- Add WebhookDeliveryService as a subscriber type
- No changes to existing subscriber interface â€” WebhookDeliveryService
  implements the existing EventSubscriber interface
```

**2.4 â€” Define acceptance criteria.**
...
This is the phase that no standalone SDD tool can replicate. Generic SDD produces a spec and moves to implementation. Architecture-grounded SDD submits the spec for validation â€” asking "what could go wrong?" with access to the full architectural context.

**3.1 â€” Check for architectural conflicts.**
...
**3.3 â€” Run a failure mode analysis.**
...
This is a **pre-mortem** â€” identifying failure modes before they become production incidents. CoreStory's answers are grounded in the actual system's failure handling patterns, queue implementations, and security model.

**3.4 â€” Revise the spec.**
...
Capture the revision in the CoreStory conversation. The diff between the original spec and the validated spec is itself valuable institutional knowledge â€” it shows what the architecture constrained.
...
Where Phase 3 asked "is this spec valid?", Phase 4 asks "where exactly does this go?" The difference from generic planning is precision: CoreStory provides exact file paths, class hierarchies, and integration points based on the actual codebase â€” not inferred from file proximity or naming conventions.

**4.1 â€” Map components to files.**
...
**4.2 â€” Identify extension points.**
...
**4.3 â€” Determine implementation order.**

```
Send CoreStory message: "What order should I implement the webhook components?
Consider actual code dependencies â€” what depends on what? What can be tested
independently?"
```
...
**Phase 4 produces:** A file-level implementation plan with exact paths, base classes, extension points, and dependency-ordered task list â€” ready for Phase 5.
...
This phase follows the same test-first methodology as the Feature Implementation playbook. The spec from Phases 1â€“4 gives the agent precise implementation guidance; this phase executes it.

**5.1 â€” Write failing tests** from the acceptance criteria in Phase 2, following patterns identified in Phase 1:

```
"Write failing tests for the webhook system based on the acceptance criteria.
Follow the existing test patterns CoreStory identified â€” use the same fixtures,
assertion styles, and test organization."
```

**5.2 â€” Implement** following the plan from Phase 4, in dependency order:
...
**5.3 â€” Validate** each component against the spec and architecture:
...
**5.4 â€” Run the full test suite** after each component. Regressions mean the implementation violated an invariant â€” go back to the CoreStory conversation and check which constraint was broken.
...
**6.2 â€” Verify invariant preservation.** Go back to the invariants from Phase 1 and confirm each one:
...
**6.4 â€” Rename the CoreStory conversation** to mark it as completed:

```
"Rename the CoreStory conversation to 'COMPLETED â€” Spec: #412 â€” Webhook notification system'"
```

This conversation is now searchable institutional knowledge. Future engineers working on the webhook system â€” or any related notification feature â€” can reference the full chain of architectural reasoning, specification decisions, and validation results. This is the living record that static spec documents can't provide.

**6.5 â€” Update the ticket** with a summary linking to the CoreStory conversation:
...
- Claude Code's YAML frontmatter requires `name` and `description` fields. The description drives auto-activation â€” make it specific.
...
- `CoreStory:create_conversation` â€” start specification thread
- `CoreStory:send_message` â€” query code intelligence
- `CoreStory:get_project_prd` â€” access product requirements
- `CoreStory:get_project_techspec` â€” access technical specifications
- `CoreStory:rename_conversation` â€” mark as completed

When instructions say "Query CoreStory", use `CoreStory:send_message`.

## Phase 1: Ground â€” Before Writing Any Spec
1. Gather requirements (from ticket or user)
2. Select CoreStory project (`CoreStory:list_projects`, verify "completed")
3. Create conversation: "Spec: #[ID] â€” [description]"
...
## Phase 2: Specify â€” Delta Specification
Write the spec constrained by Phase 1 findings:
1. **Invariants first** â€” what must NOT change
2. **Reuse section** â€” existing components to use, not recreate
3. **Delta section** â€” only new/modified components
4. **Acceptance criteria** â€” testable, referencing specific existing components

This is a delta spec: only what changes. Do NOT re-specify existing functionality.

## Phase 3: Validate â€” Architectural Pre-Mortem
...
## Phase 4: Plan â€” File-Level Implementation Map
Query CoreStory for:
...
## Phase 5: Implement â€” TDD with Validation
...
- `.github/copilot-instructions.md` is always active â€” it's global custom instructions for the project. Keep it focused on principles.
...
### Phase 2: Specify â€” Delta Specification
1. Invariants first â€” what must NOT change
2. Reuse section â€” existing components to use
3. Delta section â€” only new/modified components
...
### Phase 4: Plan â€” File-Level Map
Query CoreStory for: file paths, extension points, implementation order, test locations.

### Phase 5: Implement â€” TDD
Write failing tests â†’ implement in dependency order â†’ validate with CoreStory â†’ full suite.

### Phase 6: Verify & Capture
Verify spec compliance â†’ verify invariants â†’ commit â†’ rename conversation "COMPLETED" â†’ update ticket.
...
- For complex specs, the droid's CoreStory queries may produce long streaming responses â€” this is expected.
...
## Phase 2: Specify â€” Delta Specification
Invariants first â†’ reuse section â†’ delta (new/modified) â†’ acceptance criteria.
...
## Phase 5: Implement â€” TDD
Write failing tests â†’ implement â†’ validate with CoreStory â†’ full suite.
...
**Invariants are the spec's immune system.** The invariant section is the most valuable part of a delta specification. It tells implementers â€” human or AI â€” what they're not allowed to break. When a test fails in Phase 5, check invariants first.

**Reuse is the spec's economics.** Every existing component you reuse is a component you don't have to specify, implement, test, and maintain. The reuse-first query in Phase 1 often eliminates 30â€“50% of the specification work.
...
**Name conversations for searchability.** "Spec: #412 â€” Webhook notification system" is findable six months later. "Feature spec" is not. After completion, the "COMPLETED" prefix makes it easy to distinguish finished work from in-progress investigations.

**Don't skip Phase 3.** It's tempting to go from Specify straight to Plan â€” the spec feels done. The validation phase exists because specs always contain assumptions that don't survive contact with the real architecture. Phase 3 is where you catch conflicts before they become code review rejections or production bugs.
...
Run `CoreStory:get_project_stats` to check status. If ingestion is still running, wait for completion â€” queries against partially-ingested projects miss components and return incomplete invariants. Verify the project name matches exactly.
...
Don't drift â€” go back to the spec. If the divergence is necessary (the spec missed something), update the spec in the CoreStory conversation before continuing. The spec is a living document within the conversation, not a frozen artifact.

**CoreStory response is too long or gets cut off.**
Break your query into smaller, more specific questions. Phase 1 in particular should be 4â€“6 targeted queries, not one mega-query.
