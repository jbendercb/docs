...
- A Jira API token (generated from [id.atlassian.com/manage-profile/security/api-tokens](https://id.atlassian.com/manage-profile/security/api-tokens))
...
The Jira MCP server gives your AI agent the ability to read tickets, create issues, post comments, and transition statuses. Setup varies slightly by agent, but the core configuration is the same.
...
| Jira email | The email address associated with your Atlassian account |
...
The Jira MCP server configuration follows the same pattern across agents. Here's the core config:
...
Where to put this depends on your agent — see the Agent Configuration section below for specifics.
...
The API token inherits the permissions of the Atlassian account that generated it. The agent can only see and modify what that account can access. For most workflows, you need:
...
**2. Add Jira MCP server** to the same configuration:
...
**2. Add Jira MCP server** to the same configuration, using the JSON block from the Configuration section above.
...
**3. Add Jira MCP server** as a second custom MCP server. Configure Jira credentials as secrets.
...
Check three things: (1) the `JIRA_URL` is correct and includes `https://`, (2) the `JIRA_EMAIL` matches the account that generated the API token, and (3) the API token hasn't expired. Regenerate the token if unsure.
