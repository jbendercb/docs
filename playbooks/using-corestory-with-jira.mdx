...
**2. Add CoreStory MCP server** with Transport Type `HTTP` and your MCP URL from **Settings → IDE Integrations** as the Server URL. If Devin can complete the OAuth browser sign-in, no header is needed. If it cannot, add a [legacy token](/getting-started/mcp-server-setup#legacy-mcp-tokens-deprecated) as a secret and configure the `Authorization: Bearer $API_TOKEN` header.
