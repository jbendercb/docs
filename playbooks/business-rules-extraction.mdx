...
**How this relates to other playbooks:** This playbook extracts and documents existing rules â€” it doesn't change them. If extraction reveals rules that need to change, use the [Feature Implementation](/playbooks/feature-implementation) playbook. If you're building specifications for new features and need to understand existing rules as constraints, the [Spec-Driven Development](/playbooks/spec-driven-development) playbook's Ground phase covers that. If you're evaluating a codebase for acquisition, the [M&A Technical Due Diligence](/playbooks/ma-technical-due-diligence) playbook includes business rule assessment as one workstream.
...
- Building AI agent context â€” the business rules inventory feeds directly into `.claude/` skill files, Cursor rules, or Copilot instructions
...
- The codebase is trivially small (under ~5k LOC) â€” read it directly
...
- (Recommended) Familiarity with the codebase's domain â€” helps evaluate whether extracted rules are complete and correct
...
| 4 | Code Verification | Walk local code to verify rules, find what CoreStory missed | â€” (local code only) |
...
The core principle is **Oracle before Code**: query CoreStory for architectural understanding before navigating to specific code. CoreStory surfaces rules you wouldn't find through grep alone â€” implicit conventions, cross-layer interactions, and undocumented invariants. Code verification then grounds those discoveries in reality.
...
**A note on the PRD and TechSpec:** These documents are often very large â€” too large for an agent to hold in a single context window. Rather than trying to read them end-to-end, treat them as reference material: skim for structure and vocabulary, then use `send_message` to ask CoreStory targeted questions about specific business rules, requirements, and constraints. CoreStory has already ingested these documents and can answer questions about their contents more efficiently than the agent can parse them raw.

## Step-by-Step Walkthrough

### Phase 1 â€” Setup & Scoping
...
Identify the target project by name. Note the `project_id` â€” you'll use it for every subsequent call.
...
Parameters:
  ...
  title = "Business Rules Extraction â€” <scope description>"
```

Use a descriptive title that includes the extraction scope. Examples:
- "Business Rules Extraction â€” Order Processing Module"
- "Business Rules Extraction â€” Full System Inventory"
- "Business Rules Extraction â€” Payment & Tax Rules"
...
### Phase 2 â€” Architectural Survey (Oracle)
...
```
Tool: send_message
Query: "Where is business logic implemented in this codebase â€” is it in
...
This query produces a layer-by-layer breakdown showing where different rule types are enforced. The specifics depend on the stack â€” some examples of what you might find:
...
Note the specific file naming patterns and directory structures CoreStory identifies â€” you'll use these in Phase 4 to navigate code efficiently.
...
- **Domain vocabulary** â€” the names of entities, features, and modules as the product defines them. These become your query anchors for Phase 3.
- **Section structure** â€” which areas of the product are documented, giving you a checklist of domains to explore.
- **Explicit rule language** â€” look for sections titled "business rules," "constraints," or "validation" and for user stories that contain `business_rules` fields.
...
constraints â€” required fields, unique constraints, valid states, and
relationships between entities? Focus on constraints that encode business
rules, not just structural schema."
```

The TechSpec surfaces constraints at the data layer â€” unique fields, required relationships, enum types that define valid states. These are business rules that are often invisible to service-level analysis.
...
This gives you a category map to work through systematically in Phase 3. It's intentionally broad â€” the value is in creating a checklist of areas to explore, not in getting deep answers yet.
...
### Phase 3 â€” Deep Extraction (Oracle + Navigator)
...
- Specific constraint values (e.g., "password must be 8â€“12 characters, include 1 digit and 1 special character")
...
Calculation rules are where the most subtle business logic hides. CoreStory can map the full calculation chain â€” which is often spread across multiple utility classes and service methods.
...
Invariant queries surface the "must always be true" conditions â€” unique constraints, required relationships, format patterns. CoreStory returns these with evidence from model definitions, schema annotations, and column-level constraints.
...
This produces a step-by-step walkthrough with validation checkpoints and failure conditions at each stage â€” the most complete picture of how rules interact during a real operation.
...
This phase is essential. CoreStory provides architectural knowledge that code search alone can't match â€” but code is ground truth. Every rule extracted in Phase 3 should be verified against actual source.
...
### Phase 5 â€” Documentation & Output
...
### Phase 6 â€” Conversation Capture
...
Parameters:
  ...
  title = "RESOLVED â€” Business Rules Extraction â€” <scope description>"
```
...
- **Source Files:**
  - `path/to/file.ext` (lines XXâ€“YY) â€” [what this file does for the rule]
- **Rule Logic:**
  - [Precise statement of the rule, e.g., "Password must be 8â€“12 characters, include at least 1 digit and 1 special character"]
- **Invariants:** [What must always be true for this rule to hold]
- **Edge Cases:**
  - [Known exception or special handling]
- **Related Rules:** [BR-XXX, BR-YYY â€” cross-references to other rules]
...
```markdown
# Business Rules Inventory â€” [Project Name]
...
| BR-017 | Authorization check exists in code but not in PRD | Presumed intentional â€” needs confirmation |
...
2. Call `create_conversation` with title "Business Rules Extraction â€” <scope>"
...
3. Query CoreStory: "Based on the technical specification, what are the
   key data model constraints â€” required fields, unique constraints, valid
...
read them in full â€” they are typically too large for context. Query
...
- Search for model/entity constraint declarations (annotations, schema
  definitions, validation rules â€” patterns vary by stack)
...
## Phase 6: Capture
Rename conversation with "RESOLVED â€” Business Rules Extraction â€” <scope>"
...
GitHub Copilot supports agent skills â€” folders containing a `SKILL.md` file with YAML frontmatter that Copilot loads when relevant to a task. For project-scoped skills, store them under `.github/skills/` in your repository. For personal skills shared across projects, use `~/.copilot/skills/`.
...
2. Do NOT read the full PRD or TechSpec â€” query CoreStory about their
...
3. Do NOT try to read the full PRD or TechSpec via `get_project_prd` /
   `get_project_techspec` â€” they're too large. Query CoreStory about their
   contents via `send_message` instead.
4. Verify every CoreStory finding against local source code
...
instructions: |
  ...
  3. Query CoreStory about documented rules from the PRD and TechSpec
     (do NOT try to read the full documents â€” query via send_message)
...
1. **Best: End-to-end workflow queries.** "Show me the exact code path from X to Y. What validations happen at each step?" â€” Produces step-by-step walkthroughs with validation checkpoints and failure conditions at every stage.

2. **Excellent: Specific entity invariant queries.** "What invariants must always hold for [Entity]? What constraints exist on its configuration?" â€” Produces precise evidence from model definitions, schema constraints, and validation rules.

3. **Excellent: Domain-scoped validation queries.** "What validation rules exist for [specific entity/workflow]?" â€” Returns specific constraint values, file paths, and enforcement layers.

4. **Good: Implicit rule queries (with named edge cases).** "What implicit rules exist in [module] when [specific scenario]?" â€” Surfaces undocumented fallback behaviors. Naming specific edge cases in the query dramatically improves focus.

5. **Good: Architectural map queries.** "Where is business logic implemented â€” which layers enforce which rule types?" â€” Produces the essential orientation for targeted follow-up queries.

6. **Adequate: Broad category queries.** "What are the business rules in this system?" â€” Produces a useful high-level survey, but shallow on detail. Use this to build your Phase 2 checklist, then go deep with specific queries.
...
- When code says X but the PRD says Y, **code is ground truth** â€” document the conflict and flag it for domain expert review
- When CoreStory reports a rule that you can't find in code, it may be in dead code, a deprecated path, or a misinterpretation â€” investigate before including it
...
- After Phase 3 (before code verification) â€” to validate extracted rules against business intent
...
This usually means the codebase has little documentation and the PRD is sparse. Low confidence doesn't mean the rule is wrong â€” it means it's only evidenced in code. Flag these for domain expert validation and consider updating project documentation as a follow-up.
