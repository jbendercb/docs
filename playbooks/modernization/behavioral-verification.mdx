...
import { VerificationDashboard } from "/snippets/verification-dashboard.jsx";
...
CoreStory serves as a **Verifier** throughout this phase — comparing behavioral semantics across legacy and modernized codebases. It also operates as an **Expert** for understanding the intent behind legacy implementations: "Why does this code handle negative quantities differently from zero quantities? Is that a business rule or a bug?" This distinction matters because not every legacy behavior should be preserved — some behaviors are bugs, workarounds, or artifacts of obsolete requirements. The verification process must surface these distinctions for human judgment.
