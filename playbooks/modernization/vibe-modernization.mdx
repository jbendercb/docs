...
- **The target ingested as a second CoreStory project — once the target exists.** The dual-project setup is what lets the agent reason across both sides in a single pass. A standing target should be ingested before you begin. A greenfield target *cannot* be, and the method does not ask you to fake it: see [Step 0](#step-0-design-and-bootstrap-the-target-greenfield-only), which runs single-project and defines the point at which the second project comes online.
...
- **On the target side, direct file access substitutes cleanly while the target is small.** Code intelligence earns its keep in proportion to how much code there is to reason over; a target you can read end to end does not need an index yet. That is why Step 0 does not block on ingestion — there is genuinely nothing to gain from it yet.
...
| Tool | Step(s) | Purpose |
|------|----------|---------|
| `list_projects` | 0, 1 | Confirm which projects exist and that ingestion is complete — both legacy and target on a standing target, legacy only until [Step 0](#step-0-design-and-bootstrap-the-target-greenfield-only) hands off |
...
0. **Design and Bootstrap the Target** — *greenfield only; skip entirely when the target already exists.* Select the strategy, design the target, and build enough of it to ingest.
...
Step 0 is conditional — it runs only on a greenfield target, and a standing target starts at Step 1.

| Step | Runs on | Status in this variant |
|------|---------|------------------------|
| 0. Design and bootstrap the target | [Target Architecture & Strategy](/playbooks/modernization/target-architecture) | Borrowed — **greenfield only**, folded back in from base Phase 3; skipped outright on a standing target |
...
> **Gate 0 — Target design approval (after Step 0, greenfield only).** A human signs off the target ADR before anything is scaffolded. Inherited from base Phase 3 rather than invented here, and skipped entirely on a standing target — but on greenfield it is the cheapest gate in the arc, because changing your mind about the target costs one document instead of a delivery.
...
### Step 0: Design and Bootstrap the Target (Greenfield Only)

*Base framework: Phase 3, folded back in.*

**Skip this step entirely if the target already exists.** It exists for one situation: you have a legacy system worth modernizing and nothing yet to converge into. Running it is what makes the arc executable with a single CoreStory project.

**Select the strategy and design the target.** Run the [Target Architecture & Strategy](/playbooks/modernization/target-architecture) playbook against the legacy project — the 7 Rs, the target shape, the risk and effort call — and produce its Architectural Decision Record. Nothing here differs from the base playbook's Phase 3, so it is not restated.
...
- **The ADR is a source-grounded deliverable like any other.** Claims about what the legacy requires of the target carry `file:line` anchors against legacy source. Claims about the target are decisions, not findings — label them as such so the audit trail never confuses a choice with an observation.

⛔ **GATE 0 — target design approval.** A human signs off the ADR before scaffolding. This gate is not new; it is base Phase 3's gate, and it is the cheapest place in the entire arc to change your mind about the target.
...
**Hand off to the standing-target path.** Ingest the scaffolded target as your second CoreStory project. From this point the greenfield and standing paths are the same arc: Step 1 catalogues the target's idioms from the scaffold you just built — thin, but real, and it thickens as you go.

**Re-ingest the target on a cadence.** A greenfield target changes shape fast, and this is the one place in the arc where the intelligence layer can go stale enough to mislead. Re-ingest when the target's *structure* changes — a new module, a new layer, a new integration point — rather than on every commit. Between re-ingestions, treat the target index as a map that may be a few tickets behind, and let direct file reads settle anything structural. The always-on discipline covers you here: source decides truth, and on the target side that means the working tree.

*Base framework mapping: this step is base [Phase 3](/playbooks/code-modernization#phase-3-target-architecture-strategy) run unchanged, plus a bootstrap task the base playbook does not need because it never assumes the target is empty.*
...
In the base playbook, Phase 1 assesses the legacy for readiness and Phase 3 designs the target. When the target already exists, those two collapse: there is nothing to design, so instead of *choosing* a target you **recover the one that already exists** — and you understand the legacy well enough to converge onto it. Arriving here from [Step 0](#step-0-design-and-bootstrap-the-target-greenfield-only), you are recovering the scaffold you just built: the same procedure, against less code.
...
1. **Configure the CoreStory MCP server** in your Claude Code settings (see [CoreStory MCP Server Setup Guide](/getting-started/mcp-server-setup)). The legacy project must be ingested. Ingest the target too if it already exists — if it does not, [Step 0](#step-0-design-and-bootstrap-the-target-greenfield-only) builds it first and the arc starts with one project.
...
Design and bootstrap the target for [SystemName]   (greenfield only)
...
- If the target does not exist yet, say so in your first message. The router will start at Step 0 rather than asking for a second project it cannot have.
...
5. **A missing target project is never a reason to refuse.** Only Step 1's target half and Step 4's target half need it. If the target does not exist yet, run Step 0. If it exists but is not ingested, run single-project mode (below). Never tell the user the arc requires two projects before it can start.
...
- The target ingested as a second CoreStory project. If the target does not exist yet, this is what Step 0 produces. Do not treat its absence as a blocker at the start of the arc.
...
1. **Establish whether the target exists.** Call `list_projects` and ask the user directly. Three cases:
   - **Target exists and is ingested** → standing target. Start at Step 1; never run Step 0.
   - **Target repo exists but has little or no code** (a README, a bare directory skeleton) → greenfield. Start at Step 0. Do not ingest it yet — an empty project answers every question with "not present" and will corrupt the gap analysis.
...
3. **Check CoreStory conversations** (`list_conversations`) for completed markers:
   - "RESOLVED - [Target]..." → Step 0 complete
...
4. **If no prior work exists**, start at Step 0 for a greenfield target, Step 1 otherwise.

Once you know the step, read and follow its dedicated skill completely.

## Step 0: Design and Bootstrap the Target (greenfield only)
**Skip entirely when the target already exists.**
...
Finally, have the user ingest the scaffolded target as the second project, and stop. Step 1 runs in a new session.
...
## Error Handling
- **Only one CoreStory project exists:** do not refuse. Establish whether the target is greenfield (→ Step 0) or merely un-ingested (→ single-project mode). The arc has a defined path for both.
- **Target repo is too small to ingest, or ingests empty:** expected on greenfield. Do not retry the ingestion and do not query the empty project — bootstrap the target first per Step 0, then ingest.
...
**Single-project mode:** if the target is not ingested — greenfield, or scaffolded but not indexed — run the arc anyway, reading the target working tree in place of target-side queries. Every target claim still carries a `file:line` anchor. Exit the mode once the target is ingestable. Only Step 1's and Step 4's target halves need the second project at all.

## Step 0: Design and Bootstrap the Target (greenfield only)
Skip entirely if the target exists. Dedicated skill: `target-architecture`, run against the legacy project. Overlay: record the target's intended idioms in the ADR; target statements are decisions, not anchored findings.
⛔ GATE 0: human approves the ADR. Then scaffold to the ingestion threshold — working build, real layering, a test harness, one end-to-end vertical slice with a test — and have the user ingest it as the second project. A directory skeleton has nothing to index.
...
- Never refuse to start because only one CoreStory project exists — a greenfield target starts at Step 0, an un-ingested one runs in single-project mode
...
|---|---|---|---|
| 0. Design + bootstrap the target *(greenfield only)* | `target-architecture` (run against the legacy project) | Idioms chosen in the ADR; scaffold to the ingestion threshold, then ingest as project two | **Gate 0** — ADR approval |
...
## Single-project mode
If the target is not ingested, read the target working tree in place of target-side queries and keep every anchor. Only Step 1's and Step 4's target halves need the second project. Exit the mode once the target has real code to index.
...
**Single-project mode:** never refuse because only one project exists. A greenfield target starts at Step 0; an un-ingested one runs with direct reads of the target tree in place of target-side queries, anchors intact, until it is ingestable.

## Step map
Step 0 → `target-architecture` *(greenfield only; run against the legacy project)* ⛔ GATE 0 ADR approval, then scaffold to the ingestion threshold and ingest as project two
...
The arc does not require two projects to begin — only Step 1's target half and Step 4's target half use the second one. If the target does not exist yet, you want [Step 0](#step-0-design-and-bootstrap-the-target-greenfield-only): design it, get the ADR approved, scaffold it to the point where it has real code, then ingest it. If the target does exist but is not ingested, ingest it, or run single-project mode and read the target working tree directly in the meantime. Tell the agent which situation you are in — a router that has not been told the target is greenfield will reasonably assume it was supposed to find two.
...
**Design the target first, if there isn't one:** [Target Architecture & Strategy →](/playbooks/modernization/target-architecture) — the 7 Rs and the target ADR, run as [Step 0](#step-0-design-and-bootstrap-the-target-greenfield-only) on a greenfield target.
