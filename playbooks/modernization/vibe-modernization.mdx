...
| 2 | Rounding is half-up | CoreStory conv. `abc-123` | Opened source; contradicted | `PGM02:L88` | **Corrected** — half-even; intelligence layer was wrong |
...
This is the expected failure mode, and it is exactly what the grounding discipline exists to absorb — the fix is not a better prompt, it is a mandatory anchor. Require a `file:line` for every claim and open the cited location. Where the code contradicts the claim, the code wins and the correction goes in the Context Inventory. Treat an unusually high correction rate as information about which areas need closer human attention, not as a reason to abandon the approach.

**The intelligence layer says a module or behavior isn't there, but you know it is.**

Do not accept a negative result at face value. Absence of evidence from any retrieval layer is not evidence of absence. Re-query with different phrasing, then confirm directly against source. A false "not found" that goes unchallenged silently removes real behavior from the spec — which the orphan sweep in Step 6b is your last line of defense against.
