...
export default function VerificationDashboard() {
  ...
  Object.keys(STATUS_COLORS).forEach((s) => {
    counts[s] = RULES.filter((r) => r.status === s).length;
  });
  const total = RULES.length;

  const equivPct = Math.round((counts.Equivalent / total) * 100);
  const ready = counts.Missing === 0 && counts.Different === 0 && counts.NotVerified === 0;
  const needsReview = counts.Missing > 0 || counts.Different > 0;
  ...
  return (
    ...
    >
      ...
      <p style={{ margin: "4px 0 20px", fontSize: "13px", color: "#888" }}>
        Business rule verification for OrderService extraction · Hover over any square for details
      </p>
      ...
      <div
        style={{
          ...
          borderRadius: "8px",
          marginBottom: "20px",
          backgroundColor: ready ? "#052e16" : needsReview ? "#7f1d1d" : "#78350f",
          border: `1px solid ${ready ? "#22c55e" : needsReview ? "#ef4444" : "#f59e0b"}44`,
        }}
      >
        <span
          style={{
            fontSize: "14px",
            fontWeight: 700,
            color: ready ? "#4ade80" : needsReview ? "#fca5a5" : "#fcd34d",
          }}
        >
          {ready ? "Ready for Eliminate" : needsReview ? "Needs Remediation" : "Needs Domain Expert Review"}
        </span>
        <span style={{ fontSize: "12px", color: "#999", marginLeft: "12px" }}>
          {equivPct}% equivalent · {counts.Improved} improved · {counts.Different} different · {counts.Missing} missing · {counts.NotVerified} unverified
        </span>
      </div>
      ...
      <div style={{ display: "flex", height: "8px", borderRadius: "4px", overflow: "hidden", marginBottom: "8px" }}>
        {Object.entries(STATUS_COLORS).map(
          ([key, { bg }]) =>
            counts[key] > 0 && (
              <div key={key} style={{ width: `${(counts[key] / total) * 100}%`, backgroundColor: bg }} />
            )
        )}
      </div>
      <div style={{ display: "flex", gap: "14px", flexWrap: "wrap", marginBottom: "24px" }}>
        {Object.entries(STATUS_COLORS).map(([key, { bg, label }]) => (
          <div key={key} style={{ display: "flex", alignItems: "center", gap: "6px" }}>
            <div style={{ width: "10px", height: "10px", borderRadius: "2px", backgroundColor: bg }} />
      ...
      {AREAS.map((area) => {
        const areaRules = RULES.filter((r) => r.area === area);
        return (
          <div key={area} style={{ marginBottom: "20px" }}>
            ...
            <div style={{ display: "flex", gap: "4px", flexWrap: "wrap" }}>
              {areaRules.map((rule) => (
                <div
                  key={rule.id}
                  onMouseEnter={() => setHover(rule)}
                  onMouseLeave={() => setHover(null)}
                  style={{
                    width: "28px",
                    height: "28px",
                    borderRadius: "4px",
                    backgroundColor: STATUS_COLORS[rule.status].bg,
                    opacity: hover && hover.id === rule.id ? 1 : 0.8,
                    cursor: "pointer",
                    ...
                    color: "#fff",
                    fontWeight: 600,
                    border: hover && hover.id === rule.id ? "2px solid #fff" : "1px solid transparent",
                  }}
                >
                  {rule.id.replace("BR-", "")}
                </div>
      ...
      {hover && (
        ...
        >
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "6px" }}>
            <span style={{ fontSize: "14px", fontWeight: 700, color: "#fff" }}>{hover.id}</span>
            <span
              style={{
                fontSize: "11px",
                padding: "2px 8px",
                borderRadius: "10px",
                backgroundColor: STATUS_COLORS[hover.status].bg + "22",
                color: STATUS_COLORS[hover.status].bg,
                border: `1px solid ${STATUS_COLORS[hover.status].bg}55`,
                fontWeight: 600,
              }}
            >
              {STATUS_COLORS[hover.status].label}
            </span>
          </div>
          <div style={{ fontSize: "13px", color: "#ccc", marginBottom: hover.notes ? "8px" : 0 }}>{hover.desc}</div>
          {hover.notes && <div style={{ fontSize: "12px", color: "#999", fontStyle: "italic" }}>{hover.notes}</div>}
        </div>
      )}
      ...
      <div style={{ marginTop: "24px", borderTop: "1px solid #333", paddingTop: "16px" }}>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "12px" }}>
          <thead>
            <tr>
              <th style={{ textAlign: "left", padding: "6px 8px", color: "#888", borderBottom: "1px solid #333" }}>
                Status
              </th>
              <th style={{ textAlign: "center", padding: "6px 8px", color: "#888", borderBottom: "1px solid #333" }}>
                Count
              </th>
              <th style={{ textAlign: "center", padding: "6px 8px", color: "#888", borderBottom: "1px solid #333" }}>
                %
          ...
          <tbody>
            {Object.entries(STATUS_COLORS).map(([key, { bg, label }]) => (
              <tr key={key}>
                <td style={{ padding: "6px 8px", color: bg, borderBottom: "1px solid #222" }}>{label}</td>
                <td style={{ textAlign: "center", padding: "6px 8px", color: "#ccc", borderBottom: "1px solid #222" }}>
                  {counts[key]}
                </td>
                <td style={{ textAlign: "center", padding: "6px 8px", color: "#ccc", borderBottom: "1px solid #222" }}>
            ...
            <tr>
              <td style={{ padding: "6px 8px", color: "#fff", fontWeight: 700, borderTop: "1px solid #555" }}>Total</td>
              <td
                style={{
                  textAlign: "center",
                  padding: "6px 8px",
      ...
      <div style={{ marginTop: "16px", fontSize: "11px", color: "#555", textAlign: "center" }}>
        Sample data — replace with actual verification results from Step 5 of the Behavioral Verification playbook
      </div>
    </div>
  );
}
